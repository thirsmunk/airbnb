...
msummary(mymodeld)
```
The t value of 19.79 tells us that the number of beds is a significant predictor of the price for four nights. Our model estimates every person an apartment can accommodate adds about 481.7$ to the price of a four nights stay.
```{r}
...
Lets first look at which variables are avilable in our cleaned dataset:
```{r}
...
msummary(mymodelf)
#NOTE TO TEAM: some of there variables are suspected to be correlated but running ggpairs() offers a result that is very hard to interpret. (car::vif does not seem to work)
```
...
```{r}
#NOTE TO TEAM: If the above statement is false, please correct me
```
...
For this question, we will investigate wether the cancellation policy has an effect on the price of our four night stay. Using a similar strategy as for the previous questions, we first looked at the available variables.

```{r}
mymodeli <- lm(price_4_nights ~ cancellation_policy, data = listings_cleaned)
msummary(mymodeli)
#NOTE TO TEAM: I did not now what to adjust for so I just adjusted for nothing and in that case we find that the effect is significant. If you do know what to adjust for, just follow a method similar to what was done in all the previous cases.
```
*PUT CONCLUSION HERE ONCE WE FIGURE OUT WHICH VARIABLES TO CONTROL FOR*
